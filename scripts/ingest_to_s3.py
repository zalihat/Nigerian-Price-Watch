...
s3 = boto3.client("s3")
BUCKET_NAME = "nigeria-food-prices-bucket0003"

def download_and_upload_excel(url):
    """
    Downloads a file from a given URL and streams it directly to S3.
    """
    try:
        response = requests.get(url, stream=True)
        response.raise_for_status()
        ...
        # Stream upload to S3
        s3.upload_fileobj(
            Fileobj=response.raw,  # stream directly
            Bucket=BUCKET_NAME,
            Key=f"bronze/{filename}"
        )
